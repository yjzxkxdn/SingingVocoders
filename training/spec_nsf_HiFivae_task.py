...
from models.specHiFivae.models import Encoder, Generator, AttrDict, MultiScaleDiscriminator, MultiPeriodDiscriminator
...
class spec_nsf_HiFivae_dataset(Dataset):
    ...
    def get_data(self, index):
        ...
        if self.infer or not self.key_aug or random.random() > self.key_aug_prob:
            ...
            mel_spec_aug ,linear_spec_aug = self.mel_spec_transform(audio_aug[None, :])
            mel_spec_aug = dynamic_range_compression_torch(mel_spec_aug)
            linear_spec_aug = dynamic_range_compression_torch(linear_spec_aug)
            f0, uv = get_pitch(
                pe_name, audio, length=mel_spec_aug.shape[-1], hparams=self.config,
            ...
            mel_spec_aug = mel_spec_aug[0, :, 2:-2].T.numpy()
            linear_spec_aug = linear_spec_aug[0, :, 2:-2].T.numpy()
            f0_aug = f0[2:-2] * speed
            return {
                'f0': f0_aug,
                'linear_spec': linear_spec_aug,
                'mel_spec': mel_spec_aug,
                'audio': audio_aug
            }
    ...
    def collater(self, minibatch):
        ...
        for record in minibatch:
            ...
            if record['mel_spec'].shape[0] < crop_mel_frames:
                del record['linear_spec']
                del record['mel_spec']
                del record['audio']
                del record['f0']
                continue
            elif record['mel_spec'].shape[0] == crop_mel_frames:
                start = 0
            else:
                start = random.randint(0, record['mel_spec'].shape[0] - 1 - crop_mel_frames)
            ...
            if self.infer:
                cty = (len(record['mel_spec'].T) * samples_per_frame)
                record['audio'] = record['audio'][:cty]
                record['audio'] = np.pad(record['audio'], (
                    0, (len(record['mel_spec'].T) * samples_per_frame) - len(record['audio'])),
                                         mode='constant')
                pass
            else:
        ...
        if self.volume_aug:
            for record in minibatch:
                if record.get('audio') is None:
                    continue
                audio = record['audio']
                audio_mel = record['mel_spec']
                audio_linear = record['linear_spec']
                ...
                if random.random() < self.volume_aug_prob:
                    ...
                    audio_linear += log_mel_shift

                audio_mel = torch.clamp(torch.from_numpy(audio_mel), min=np.log(1e-5)).numpy()
                audio_linear = torch.clamp(torch.from_numpy(audio_linear), min=np.log(1e-5)).numpy()
                record['audio'] = audio
                record['mel_spec'] = audio_mel
                record['linear_spec'] = audio_linear
...
class spec_nsf_HiFivae(GanBaseTask):
    ...
    def build_dataset(self):
        ...
        self.train_dataset = spec_nsf_HiFivae_dataset(
            config=self.config,
            data_dir=pathlib.Path(self.config['DataIndexPath']) / self.config['train_set_name']
        )

        self.valid_dataset = spec_nsf_HiFivae_dataset(
            config=self.config,
    ...
    def build_model(self):
        ...
        encoder_cfg = self.config['e_model_args']
        gh = AttrDict(cfg)
        eh = AttrDict(encoder_cfg)
        self.encoder = Encoder(eh)
        self.generator = Generator(gh)
    ...
    def load_pre_train_model(self):
        base_state_dict = super().load_pre_train_model()

        encoder_ckpt_path = self.config.get('encoder_ckpt_path')
        if encoder_ckpt_path is not None:
            encoder_ckpt = torch.load(encoder_ckpt_path)
            
            for key, param in encoder_ckpt['state_dict'].items():
                base_state_dict[key] = param
    ...
    def Gforward(self, sample):
        ...
        """
        z, m, logs = self.encoder(sample['linear_spec'])
        wav = self.generator(x=z, f0=sample['f0'])
    ...
    def G2forward(self, sample, pc_aug_num):
        ...
        wav_mixed = self.generator(x=z, f0=torch.cat((f0_shift_c, f0[pc_aug_num:]), dim=0))
        wav_shift_c, wav_shift_0 = wav_mixed[:pc_aug_num], wav_mixed[pc_aug_num:]
        _, spec_shift_c = self.TF(wav_shift_c.squeeze(1))
        spec_shift_c = self.TF.dynamic_range_compression_torch(spec_shift_c)
        z_shift_c = self.encoder(spec_shift_c)[0]
        wav_shift_back = self.generator(x=z_shift_c, f0=f0[:pc_aug_num])
        ...
        wav_shift_a = self.generator(x=z[:pc_aug_num], f0=f0_shift_a)
        _, spec_shift_a = self.TF(wav_shift_a.squeeze(1))
        spec_shift_a = self.TF.dynamic_range_compression_torch(spec_shift_a)
        z_shift_a = self.encoder(spec_shift_a)[0]
        wav_shift_ab = self.generator(x=z_shift_a, f0=f0_shift_c)
    ...
    def _training_step(self, sample, batch_idx):
        ...
        kl_loss = self.kl_loss(Goutput['lossxxs'][1], Goutput['lossxxs'][2]) * self.config['kl_loss_weight']
        if pc_aug:
        ...
        Auxloss = spec_loss + pc_wav_loss + kl_loss
